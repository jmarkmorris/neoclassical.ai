...
**Compatibility with the $\beta$-lock framework:** These derivations use the action law only and do **not** enforce the middle-branch lock ($v_M=c_f$). To combine them with the lock, interpret the result as a $\beta(f)$ scaling for the inner/outer branches, while the middle branch stays fixed at $v_M=c_f$ with $\beta(f_P)=1$ at the horizon. To align with the $\beta$-form above, use
$$
r(f) = \frac{\beta(f)\,c_f}{2\pi f},
$$
and interpret the resulting $r(f)$ relations as *branch-specific scaling laws* for $\beta(f)$. The middle branch remains fixed at $\beta(2f)=1$, and the outer branch is normalized so $\beta(f_P)=1$ at the horizon.
