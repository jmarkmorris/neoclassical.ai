...
2.  **Field Speed Lock (hinge condition):** $v_M = c_f$ is invariant (so $\beta(f_M)=1$); $f_M$ and $r_M$ may co-vary along the hyperbola $2\pi f_M r_M = c_f$, but the velocity stays pinned to $c_f$. (When we also set $v_O=c_f$, the same relation holds for the Outer binary, i.e., $\beta(f_O)=1$ under the lock.)
...
Take the horizon state as the reference: Outer and Middle ride $c_f$ (our hinge), while the Inner is already in the self-hit regime at the low-frequency floor with $v_I = \beta(f_I) c_f$, $\beta>1$. As $f_O$ increases, $\beta(f_I)$ grows within the self-hit region and is capped at the universal MCB value $\beta_{\text{MCB}}>1$. With the 1:2:4 lock,
$$
r_I = \frac{v_I}{2\pi f_I} = \frac{\beta(f_I) c_f}{8\pi f_O} = \beta(f_I) \left(\frac{1}{4} r_O\right),
$$
so the Inner sits at the geometric $r_O/4$ only if $\beta=1$; the self-hit condition ($\beta>1$) inflates it by the factor $\beta$. The MCB is a single, universal limit state: binaries may sit below it, but no binary can surpass it. The offset $\beta-1$ remains the candidate for core binding energy.
...
MM: How does this formula for radius compare to standard $\Lambda\mathrm{CDM}$ inflation and expansion curves?
...
**Detail Table (1:2:4 lock across the full range; $v_k = \beta(f_k)c_f$, $\beta(f_P)=1$):**

| $f_I$ | $f_M$ | $f_O$ | $r_I$ | $r_M$ | $r_O$ | $v_I$ | $v_M$ | $v_O$ |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| $4\ \text{Hz}$ | $2\ \text{Hz}$ | $1\ \text{Hz}$ | $\dfrac{\beta(4\,\text{Hz})\,c_f}{8\pi}$ | $\dfrac{\beta(2\,\text{Hz})\,c_f}{4\pi}$ | $\dfrac{\beta(1\,\text{Hz})\,c_f}{2\pi}$ | $\beta(4\,\text{Hz})\,c_f$ | $\beta(2\,\text{Hz})\,c_f$ | $\beta(1\,\text{Hz})\,c_f$ |
| $4f$ | $2f$ | $f$ with $1\,\text{Hz} < f < f_P$ | $\dfrac{\beta(4f)\,c_f}{8\pi f}$ | $\dfrac{\beta(2f)\,c_f}{4\pi f}$ | $\dfrac{\beta(f)\,c_f}{2\pi f}$ | $\beta(4f)\,c_f$ | $\beta(2f)\,c_f$ | $\beta(f)\,c_f$ |
| $4f_P$ | $2f_P$ | $f_P$ | $\dfrac{\beta(4f_P)\,c_f}{8\pi f_P}$ | $\dfrac{\beta(2f_P)\,c_f}{4\pi f_P}$ | $\dfrac{\beta(f_P)\,c_f}{2\pi f_P}$ | $\beta(4f_P)\,c_f$ | $\beta(2f_P)\,c_f$ | $c_f$ |
