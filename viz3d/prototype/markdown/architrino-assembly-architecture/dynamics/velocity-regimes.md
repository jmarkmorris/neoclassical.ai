...
This section defines the three velocity regimes in the architrino framework and how they map onto tri-binary structure.

## Regime Definitions

- **Sub-field-speed** ($v<c_f$): No self-hit; outer-binary expansion/contraction modes dominate.
- **Field-speed** ($v=c_f$): Symmetry-breaking threshold; **middle binary always rides field speed** with **variable radius and frequency** and acts as the **energy-storage fulcrum**.
- **Super-field-speed** ($v>c_f$): Self-hit regime; maximal-curvature stabilization in the inner binary.
