...
- Point emission at $(t_0, \mathbf{s}_0)$:
  $$
...
Given a receiver $o'$ at time $t$ and a source $j$ at causal emission time $t_0 \in \mathcal{C}_j(t)$, let
$$
...
Plain language: For each past emission that can reach you now, push along the line back to where it came from, with $1/r^2$ falloff only, then add all pushes.
...
## 10) Markdown math rendering (VSCode)

- Use `$...$` for inline math and `$$...$$` for display math.
- Do not use `\(...\)` or `\[...\]` because VSCode math preview is inconsistent with those.
- Keep math ASCII. Replace Unicode superscripts/subscripts with LaTeX: `$10^{-24}$`, `$cm^3$`, `$N_2$`, `$H_2O$`.
- Treat caret notation as exponentiation and wrap in math: `$10^{60}$`, not `10^60`.
- Words in subscripts use `\text{...}`: `$t_{\text{emit}}$`, `$x_{\text{emitter}}$`.
- Use braces for multi-character subscripts/superscripts: `$r_{\text{hit}}$`, `$10^{12}$`.
- Use math for plain expressions in prose: `$1/r^2$`, `$E=mc^2$`, `$H(0)=0$`.
- For a capital Lambda followed by text, include a space: `$\\Lambda$ CDM` or `$\Lambda\ \mathrm{CDM}$`, not `\LambdaCDM`.
- Keep hyphenated words out of math; wrap only the symbols: `Lambda-CDM` as `$\\Lambda$-CDM`.
