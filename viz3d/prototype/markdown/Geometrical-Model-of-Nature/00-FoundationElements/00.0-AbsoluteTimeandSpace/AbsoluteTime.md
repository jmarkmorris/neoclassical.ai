...
**Retarded Potentials:** If a source is located at $(\hat{t}_0, \mathbf{x}_0)$, its influence reaches a receiver at $(\hat{t}, \mathbf{x})$ at the **history time**:
$$
...
**In particular, the interaction law (defined in the dynamics section) is built entirely from history contributions at times $t' < t$; the model contains no advanced or instantaneous interaction terms.** This ensures causality at the fundamental level.
...
At time $t$, an architrino at position $\mathbf{x}(t)$ experiences forces from all other architrinos based on the **intersection of its worldline with spherical shells** emitted at all past times $t' < t$. This is naturally encoded in the history interaction law and gives rise to **non-Markovian memory effects** (e.g., the self-hit regime, where an architrino interacts with its own past emissions).
