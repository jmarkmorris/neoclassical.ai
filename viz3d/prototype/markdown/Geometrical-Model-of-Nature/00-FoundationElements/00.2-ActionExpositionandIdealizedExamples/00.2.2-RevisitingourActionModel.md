...
- Method 1 (PDE): whole-field grid simulations, visualization, and complex media/boundaries. Deposit a smeared source each step; robust when an emitter slows or stops. Aggregate particle data to coarse-grained densities $n(x,t)$, $\rho(x,t)$, and $\mathcal{E}(x,t)$ as inputs/targets for PDE runs and validation.
...
- Method 2: Causality and superposition are exact; amplitudes are $1/(4\pi r)$ with a Jacobian $|1-\mathbf{n}\cdot\mathbf{v}_s/c|^{-1}$ when evaluating the path-history time delta. To preserve constant per-wavefront amplitude in the canonical law, geometric factors are absorbed into $\kappa$ when comparing accelerations.
...
- Method 2: Exact potentials in free space; gradients give forces; care is needed near $|1-\mathbf{n}\cdot\mathbf{v}_s/c| \to 0$ geometries.
- Method 3: Energetics are validated via $\eta$-mollified potentials $\Phi_\eta$ and work–energy on resolved windows; impulses are recovered as $\eta \to 0$ in the weak sense.
...
# Layered penetration diagram (molecules $\to$ cores)
...
| Layer | Photons | Neutrinos | Charged $\pm\epsilon$ | Dark-matter-like neutral |
| --- | --- | --- | --- | --- |
