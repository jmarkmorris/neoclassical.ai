...
*   Investigate Godot's capabilities for creating reusable code libraries or modules (e.g., Autoloads, custom `class_name` scripts, potentially C# libraries or GDNative/GDExtension for more complex needs).
*   Explore helper scripts for tasks like batch processing assets or automating parts of the build/export pipeline.
*   Evaluate the effectiveness of multimodal AI (like Gemini) for understanding visual aspects by providing screenshots or diagrams.
